...
sparse formats must include metadata overhead. Sparse-compute capabilities must match an exact
operator kind, dtype, and pattern and must identify their performance source. New optimization paths
must retain the fail-closed quality gate; do not substitute quantization MSE for task accuracy unless
that is explicitly the task metric.
